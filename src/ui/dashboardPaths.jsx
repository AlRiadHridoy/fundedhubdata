<div className="faq-nav-wrap bg-white xl:px-[56px] lg:px-[30px] py-[48px] overflow-y-scroll scrollbar-gray dark:bg-dark">
  {/* faq */}
  <div className="faq-toggle-wrap border-[#F2F2F2] border-[1px] shadow-[0px_2.13333px_90.6667px_rgba(217,225,255,0.59)] my-2 dark:bg-toggle dark:shadow-[0px_2.13333px_10.6667px_rgba(217,225,255,0.59)]">
    <div className="faq-toggle-head flex items-center justify-between py-[16px] px-[32px] cursor-pointer">
      <h4 className="text-dark text-[18px] font-semibold dark:text-white">
        Express Rule
      </h4>
      <svg
        width="11"
        height="7"
        viewBox="0 0 11 7"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="fill-dark dark:fill-white"
      >
        <path d="M5.00409 6.00409C4.87249 6.00485 4.74202 5.97963 4.62019 5.92986C4.49835 5.8801 4.38753 5.80677 4.29409 5.71409L0.294092 1.71409C0.200853 1.62085 0.126893 1.51016 0.0764322 1.38834C0.0259719 1.26652 1.96485e-09 1.13595 0 1.00409C-1.96485e-09 0.872232 0.0259719 0.741664 0.0764322 0.619842C0.126893 0.49802 0.200853 0.38733 0.294092 0.294091C0.38733 0.200853 0.498021 0.126892 0.619843 0.0764313C0.741665 0.0259709 0.872233 -1.96485e-09 1.00409 0C1.13595 1.96486e-09 1.26652 0.0259709 1.38834 0.0764313C1.51016 0.126892 1.62085 0.200853 1.71409 0.294091L5.00409 3.60409L8.30409 0.424091C8.39608 0.321802 8.50819 0.239593 8.6334 0.182607C8.75862 0.125621 8.89424 0.0950833 9.03179 0.0929041C9.16935 0.0907248 9.30587 0.11695 9.43283 0.169941C9.55978 0.222932 9.67444 0.301549 9.76962 0.400873C9.86481 0.500198 9.93848 0.618093 9.98602 0.747188C10.0336 0.876282 10.054 1.0138 10.0459 1.15113C10.0379 1.28847 10.0016 1.42267 9.93936 1.54535C9.8771 1.66802 9.7902 1.77653 9.68409 1.86409L5.68409 5.72409C5.50122 5.90041 5.25809 6.00052 5.00409 6.00409Z"></path>
      </svg>
    </div>
    <div className="faq-toggle-body py-[16px] px-[40px]">
      <button className="w-full text-left mb-[16px] relative pl-[25px] lg:text-[18px] text-[16px] before:content-[''] before:w-[10px] before:h-[10px] before:rounded-full before:absolute before:left-0 before:top-[8px]">
        DOES UWM OPERATE 24/7?
      </button>
      <button className="w-full text-left mb-[16px] relative pl-[25px] lg:text-[18px] text-[16px] before:content-[''] before:w-[10px] before:h-[10px] before:rounded-full before:absolute before:left-0 before:top-[8px]">
        WHAT IS THE MAXIMUM FUNDING A TRADER CAN GET?
      </button>
    </div>
  </div>

  {/* faq */}
  <div className="faq-toggle-wrap border-[#F2F2F2] border-[1px] shadow-[0px_2.13333px_90.6667px_rgba(217,225,255,0.59)] my-2 dark:bg-toggle dark:shadow-[0px_2.13333px_10.6667px_rgba(217,225,255,0.59)]">
    <div className="faq-toggle-head flex items-center justify-between py-[16px] px-[32px] cursor-pointer dark:bg-toggle">
      <h4 className="text-dark text-[18px] font-semibold dark:text-white">
        Evaluation Rule
      </h4>
      <svg
        width="11"
        height="7"
        viewBox="0 0 11 7"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="fill-dark dark:fill-white"
      >
        <path d="M5.00409 6.00409C4.87249 6.00485 4.74202 5.97963 4.62019 5.92986C4.49835 5.8801 4.38753 5.80677 4.29409 5.71409L0.294092 1.71409C0.200853 1.62085 0.126893 1.51016 0.0764322 1.38834C0.0259719 1.26652 1.96485e-09 1.13595 0 1.00409C-1.96485e-09 0.872232 0.0259719 0.741664 0.0764322 0.619842C0.126893 0.49802 0.200853 0.38733 0.294092 0.294091C0.38733 0.200853 0.498021 0.126892 0.619843 0.0764313C0.741665 0.0259709 0.872233 -1.96485e-09 1.00409 0C1.13595 1.96486e-09 1.26652 0.0259709 1.38834 0.0764313C1.51016 0.126892 1.62085 0.200853 1.71409 0.294091L5.00409 3.60409L8.30409 0.424091C8.39608 0.321802 8.50819 0.239593 8.6334 0.182607C8.75862 0.125621 8.89424 0.0950833 9.03179 0.0929041C9.16935 0.0907248 9.30587 0.11695 9.43283 0.169941C9.55978 0.222932 9.67444 0.301549 9.76962 0.400873C9.86481 0.500198 9.93848 0.618093 9.98602 0.747188C10.0336 0.876282 10.054 1.0138 10.0459 1.15113C10.0379 1.28847 10.0016 1.42267 9.93936 1.54535C9.8771 1.66802 9.7902 1.77653 9.68409 1.86409L5.68409 5.72409C5.50122 5.90041 5.25809 6.00052 5.00409 6.00409Z"></path>
      </svg>
    </div>
    <div className="faq-toggle-body py-[16px] px-[40px]">
      <button className="w-full text-left mb-[16px] relative pl-[25px] lg:text-[18px] text-[16px] before:content-[''] before:w-[10px] before:h-[10px] before:rounded-full before:absolute before:left-0 before:top-[8px]">
        HOW I CAN EDIT SMART OBJECTS ?
      </button>
      <button className="w-full text-left mb-[16px] relative pl-[25px] lg:text-[18px] text-[16px] before:content-[''] before:w-[10px] before:h-[10px] before:rounded-full before:absolute before:left-0 before:top-[8px]">
        CAN I GET A RETRY WITHOUT PAYING? ARE THERE ANY EXTENSIONS IN PLACE?
      </button>
      <button className="w-full text-left mb-[16px] relative pl-[25px] lg:text-[18px] text-[16px] before:content-[''] before:w-[10px] before:h-[10px] before:rounded-full before:absolute before:left-0 before:top-[8px]">
        DO I ACTUALLY TRADE ON A LIVE ACCOUNT?
      </button>
    </div>
  </div>
</div>;
